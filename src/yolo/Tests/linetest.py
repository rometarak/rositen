...
import numpy as np
import cv2
# roi = gray_image[:, width//2:]  #right side image, random to keep here
def process_frame(color_frame):
    """Processes the frame to detect white lines."""
    color_image = np.asanyarray(color_frame.get_data())
    
    gray_image = cv2.cvtColor(color_image, cv2.COLOR_BGR2GRAY)
    _, thresholded = cv2.threshold(gray_image, 125, 255, cv2.THRESH_BINARY)
    ...
    if lines is not None:
        for x1, y1, x2, y2 in lines[:, 0]:
            ...
            cv2.line(gray_image, (x1, y1), (x2, y2), (0, 255, 0), 2)
            cv2.putText(gray_image, f"({x1},{y1}) -> ({x2},{y2})", (x1, y1 - 10),
